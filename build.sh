...
check_internet() {
    ...
    if command -v curl &>/dev/null; then
        if curl -s --head --request HEAD http://www.google.com | grep "200 OK" > /dev/null; then
            IS_ONLINE=true
        fi
...
add_package "kmod-br-netfilter" # Allows firewall rules to apply to traffic passing between LAN ports, VLAN
...
# Ensure bridger (for Mediatek) is not included
sed -i '/CONFIG_PACKAGE_bridger/d' .config
echo "# CONFIG_PACKAGE_bridger is not set" >> .config
